...
//MedfieldOS class Implementation
MedfieldOS::MedfieldOS()
{
    m_dnx_os_name = NULL;
...
bool MedfieldOS::Init(char* dnx_os_name, char* os_image_name, 
...
dnx_data* MedfieldOS::GetNoSizeData()
{
    bool ret = true;
...
bool MedfieldOS::CheckFile(char *filename)
{
    bool ret = true;
    this->m_utils->u_log(LOG_ENTRY, "%s", __FUNCTION__);
    FILE *fp;
...
unsigned long MedfieldOS::GetFileSize(char* file)
{
	size_t lSize = 0;
    this->m_utils->u_log(LOG_ENTRY, "%s", __FUNCTION__);
...
dnx_data* MedfieldOS::GetOsipHdr()
{
    bool ret = true;
...
dnx_data* MedfieldOS::GetOsImageDataChunk()
{
    this->m_utils->u_log(LOG_ENTRY, "%s", __FUNCTION__);
...
void MedfieldOS::ReleaseOsImageDataChunk()
{
    this->m_utils->u_log(LOG_ENTRY, "%s", __FUNCTION__);
    if(m_pkt_buffer)
        delete[] m_pkt_buffer;
...
dnx_data* MedfieldOS::GetOsDnXHdr()
{
    this->m_utils->u_log(LOG_ENTRY, "%s", __FUNCTION__);
    int ret_code = 0;
...
unsigned long long MedfieldOS::GetOsImageDataSize()
{
    return m_os_data_size;
}

void MedfieldOS::LogError(int errorcode)
{
    this->m_utils->u_log(LOG_ENTRY, "%s", __FUNCTION__);
