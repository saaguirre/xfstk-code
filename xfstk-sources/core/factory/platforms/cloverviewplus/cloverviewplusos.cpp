...
//CloverviewPlusOS class Implementation
CloverviewPlusOS::CloverviewPlusOS()
{
    m_dnx_os_name = NULL;
...
bool CloverviewPlusOS::Init(char* dnx_os_name, char* os_image_name,
...
dnx_data* CloverviewPlusOS::GetNoSizeData()
{
    bool ret = true;
...
bool CloverviewPlusOS::CheckFile(char *filename)
{
    bool ret = true;
    this->m_utils->u_log(LOG_ENTRY, "%s", __FUNCTION__);
    FILE *fp;
...
bool CloverviewPlusOS::InitOsipHdr()
{
    this->m_utils->u_log(LOG_ENTRY, "%s", __FUNCTION__);
...
bool CloverviewPlusOS::InitOsImage()
{
    int ret_code = 0;
...
unsigned long CloverviewPlusOS::GetFileSize(char* file)
{
	size_t lSize = 0;
    this->m_utils->u_log(LOG_ENTRY, "%s", __FUNCTION__);
...
dnx_data* CloverviewPlusOS::GetOsipHdr()
{
    bool ret = true;
...
dnx_data* CloverviewPlusOS::GetOsImageDataChunk()
{
    this->m_utils->u_log(LOG_ENTRY, "%s", __FUNCTION__);
...
void CloverviewPlusOS::ReleaseOsImageDataChunk()
{
    this->m_utils->u_log(LOG_ENTRY, "%s", __FUNCTION__);
    if(m_pkt_buffer)
        delete[] m_pkt_buffer;
...
dnx_data* CloverviewPlusOS::GetOsDnXHdr()
{
    this->m_utils->u_log(LOG_ENTRY, "%s", __FUNCTION__);
    int ret_code = 0;
...
dnx_data* CloverviewPlusOS::GetOsDnX()
{
    this->m_utils->u_log(LOG_ENTRY, "%s", __FUNCTION__);
...
unsigned long long CloverviewPlusOS::GetOsImageDataSize()
{
    return m_os_data_size;
}

void CloverviewPlusOS::LogError(int errorcode)
{
    this->m_utils->u_log(LOG_ENTRY, "%s", __FUNCTION__);
