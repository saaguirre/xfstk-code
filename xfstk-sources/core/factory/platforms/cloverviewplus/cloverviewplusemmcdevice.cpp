...
int CloverviewPlusEmmcDevice::GetNumberAvailableDevices()
{
    int RetVal = 0;
...
int CloverviewPlusEmmcDevice::GetNumberAvailableDevices(DeviceInfos* socdevices)
{
    int RetVal = 0;
...
#endif
bool CloverviewPlusEmmcDevice::SetUtilityInstance(void *UtilityInstance)
{
    int RetVal = 0;
...
void* CloverviewPlusEmmcDevice::GetDeviceHandle(int DeviceNumber)
{
    void *RetVal = NULL;
    RetVal = this->PhysicalTransportDevice->GetDeviceHandle(DeviceNumber);
    return RetVal;
}
bool CloverviewPlusEmmcDevice::SetDeviceHandle(void *DeviceHandle)
{
    bool RetVal = false;
    RetVal = this->PhysicalTransportDevice->SetDeviceHandle(DeviceHandle);
    return RetVal;
}
bool CloverviewPlusEmmcDevice::SetTransport(DeviceTransportType Transport)
{
    bool RetVal = false;
    switch(Transport) {
    case XFSTK_USB20:
        ...
        break;
#else
        this->PhysicalTransportDevice = (IDevice *) new CloverviewPlusEmmcUSB20Device();
        if(this->PhysicalTransportDevice) {
            RetVal = true;
        }
        break;
#endif
    case XFSTK_SERIAL:
        this->PhysicalTransportDevice = (IDevice *) new CloverviewPlusEmmcSerialDevice();
        if(this->PhysicalTransportDevice) {
            RetVal = true;
        }
        break;
...
bool CloverviewPlusEmmcDevice::Open()
{
    bool RetVal = false;
...
bool CloverviewPlusEmmcDevice::Init()
{
    bool RetVal = false;
...
bool CloverviewPlusEmmcDevice::Abort()
{
    bool RetVal = false;
...
bool CloverviewPlusEmmcDevice::Write(void *pBuffer, unsigned int bufferSize)
{
    bool RetVal = false;
...
bool CloverviewPlusEmmcDevice::Read(void *pBuffer, unsigned int bufferSize)
{
    bool RetVal = false;
...
bool CloverviewPlusEmmcDevice::GetAck(void *pAck, unsigned int *ackSize)
{
    bool RetVal = false;
...
int CloverviewPlusEmmcDevice::Detect()
{   int RetVal = 0;
    RetVal = this->PhysicalTransportDevice->Detect();
    return RetVal;
}

bool CloverviewPlusEmmcDevice::IsCorrectProtocol(void *szBuff, unsigned int *bytes_rxed)
{   bool RetVal = false;
    RetVal = this->PhysicalTransportDevice->IsCorrectProtocol(szBuff, bytes_rxed);
    return RetVal;
}

bool CloverviewPlusEmmcDevice::IsSupportedDevice(unsigned short vendorid, unsigned short productid)
{   bool RetVal = false;
    RetVal = this->PhysicalTransportDevice->IsSupportedDevice(vendorid, productid);
    return RetVal;
}

void CloverviewPlusEmmcDevice::GetUsbsn(char * usbsn)
{
...
//////////////////////////////////////
CloverviewPlusEmmcUSB20Device::CloverviewPlusEmmcUSB20Device()
{
    this->bus = NULL;
    this->dev = NULL;
    this->dev_port = NULL;
...
int CloverviewPlusEmmcUSB20Device::GetNumberAvailableDevices()
{
...
#if defined XFSTK_OS_WIN
int CloverviewPlusEmmcUSB20Device::GetNumberAvailableDevices(DeviceInfos* socdevices)
{
    int RetVal = 0;
    return RetVal;
}
#endif
void* CloverviewPlusEmmcUSB20Device::GetDeviceHandle(int DeviceNumber)
{
    struct usb_device *dev = NULL;
    struct usb_bus *bus = NULL;
    int availnumdevs = 0;
...
bool CloverviewPlusEmmcUSB20Device::SetDeviceHandle(void *DeviceHandle)
{
    bool RetVal = false;
    int ret = 0;
...
bool CloverviewPlusEmmcUSB20Device::VerifyDeviceHandle(struct usb_device* Handle)
{
    struct usb_device *dev = NULL;
    struct usb_bus *bus = NULL;
    char myusbsn[128];
...
void CloverviewPlusEmmcUSB20Device::FindDevice(void)
{
...
bool CloverviewPlusEmmcUSB20Device::Init()
{
    bool RetVal = true;
    this->dev_handle = NULL;
    this->in_pipe    = 0;
...
bool CloverviewPlusEmmcUSB20Device::SetUtilityInstance(void *UtilityInstance)
{
    if(UtilityInstance) {
...
bool CloverviewPlusEmmcUSB20Device::Abort()
{
    this->libutils->u_log(LOG_USB, "%s", __PRETTY_FUNCTION__);
...
bool CloverviewPlusEmmcUSB20Device::Write(void *pBuffer, unsigned int bufferSize)
{
    bool RetVal = true;
    ...
    for(ULONG i = 0; i < delay_time; i++){
        bytes_written = usb_bulk_write(dev_handle, out_pipe, reinterpret_cast<char*>(pBuffer),
                                      bufferSize, CloverviewPlusEmmcUSB20Device::USBTIMEOUT);
        if(bytes_written < 0) {
            bytes_written = 0;
            return false;
        }
        ...
        if(i == 5) {
            ...
            //usb_clear_halt(dev_handle, out_pipe);
            return false;
        }
...
bool CloverviewPlusEmmcUSB20Device::Read(void *szBuff, unsigned int bufferSize)
{
    bool RetVal = true;
    int bytes_rxed = 0;
    ULONG delay_time = 2000;//USBTIMEOUT;
...
bool CloverviewPlusEmmcUSB20Device::GetAck(void *szBuff, unsigned int *bytes_rxed)
{
    bool RetVal = true;
    ULONG delay_time = 2000;//USBTIMEOUT;
    if(dev_handle == NULL) {
    ...
    for(ULONG i = 0; i < delay_time; i++){
        *bytes_rxed = usb_bulk_read(dev_handle, in_pipe, reinterpret_cast<char*>(szBuff),
                                  512, CloverviewPlusEmmcUSB20Device::USBTIMEOUT);
        if ((*bytes_rxed) < 0) {
           this->libutils->u_log(LOG_USB, "usb_bulk_read() fails");
           RetVal = false;
        }
...
int CloverviewPlusEmmcUSB20Device::Detect()
{
    bool RetVal = false;
    return 0;
}
bool CloverviewPlusEmmcUSB20Device::IsCorrectProtocol(void *szBuff, unsigned int *bytes_rxed)
{
    bool RetVal = false;
    if(!this->libutils) {
        return RetVal;
...
bool CloverviewPlusEmmcUSB20Device::IsSupportedDevice(unsigned short vendorid, unsigned short productid)
{
...
void CloverviewPlusEmmcUSB20Device::GetUsbsn(char * usbsn)
{
    //this->libutils->u_log( LOG_USB, "USBSN: %s", this->usbsn);
...
#endif
//////////////////////////////////////////
CloverviewPlusEmmcSerialDevice::CloverviewPlusEmmcSerialDevice()
{
}
int CloverviewPlusEmmcSerialDevice::GetNumberAvailableDevices()
{
    return 0;
}
bool CloverviewPlusEmmcSerialDevice::SetUtilityInstance(void *UtilityInstance)
{
    if(UtilityInstance) {
        return true;
    }
...
void* CloverviewPlusEmmcSerialDevice::GetDeviceHandle(int DeviceNumber)
{
    return NULL;
}

bool CloverviewPlusEmmcSerialDevice::SetDeviceHandle(void *DeviceHandle)
{
    bool RetVal = false;
    if(DeviceHandle != NULL) {
        this->CurrentDeviceHandle = DeviceHandle;
...
bool CloverviewPlusEmmcSerialDevice::Open()
{
    bool RetVal = false;
    return RetVal;
}

bool CloverviewPlusEmmcSerialDevice::Init()
{
    bool RetVal = false;
    return RetVal;
}

bool CloverviewPlusEmmcSerialDevice::Abort()
{
    return true;
}

bool CloverviewPlusEmmcSerialDevice::Write(void *pBuffer, unsigned int bufferSize)
{
    bool RetVal = false;
    return RetVal;
}

bool CloverviewPlusEmmcSerialDevice::Read(void *szBuff, unsigned int bufferSize)
{
    bool RetVal = false;
    return RetVal;
}

bool CloverviewPlusEmmcSerialDevice::GetAck(void *szBuff, unsigned int *bytes_rxed)
{
    bool RetVal = false;
    return RetVal;
}

int CloverviewPlusEmmcSerialDevice::Detect()
{
    return 0;
}
bool CloverviewPlusEmmcSerialDevice::IsCorrectProtocol(void *szBuff, unsigned int *bytes_rxed)
{
    bool RetVal = false;
    return RetVal;
}

void CloverviewPlusEmmcSerialDevice::GetUsbsn(char * usbsn)
{

}

bool CloverviewPlusEmmcSerialDevice::IsSupportedDevice(unsigned short vendorid, unsigned short productid)
{
    return true;
}
