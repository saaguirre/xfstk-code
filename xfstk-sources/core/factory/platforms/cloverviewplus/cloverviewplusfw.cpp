...
//CloverviewPlusFW Implementation
CloverviewPlusFW::CloverviewPlusFW()
{
    m_fname_dnx_fw = NULL;
...
CloverviewPlusFW::~CloverviewPlusFW()
{
    ...
    for(int i = 0; i < NUM_FW_DATA_CLVP; i++) {
       if(fw_data_set[i])
             ...
             unsigned long gpflags, bool enable_ifwi_wipe, bool perform_emmc_dump)
{
    ...
    if(InitNoSize()) {
        fw_data_set[FW_DATA_DNX0H_CLVP]->size = DNX_FW_SIZE_HDR_SIZE;
        fw_data_set[FW_DATA_DNX0H_CLVP]->data = m_dnx_os_size_hdr;
    }
    ...
    fw_data_set[FW_DATA_DNXD_CLVP]->size = m_dnx_fw_size;
    fw_data_set[FW_DATA_DNXD_CLVP]->data = m_dnx_fw;
    fw_data_set[FW_DATA_DNX0H_CLVP]->size = m_dnx_fw_header_size;
    fw_data_set[FW_DATA_DNX0H_CLVP]->data = m_dnx_os_size_hdr;
    return ret;
}

bool CloverviewPlusFW::InitDnxHdr()
{
    int ret_code = 1;
    bool ret = true;
	...
	catch( std::bad_alloc& )
	{
        ...
        LogError(ret_code);
        this->m_utils->u_log(LOG_FWUPGRADE,"%s Exception raised: ErrorCode %d", __FUNCTION__, ret_code);
...
void CloverviewPlusFW::InitFuphHeaderData()
{
...
bool CloverviewPlusFW::FindFuphHeaderSignature()
{
    const UINT fuphSignature = 0x55504824;//UPH$
    m_fw_image_size = this->m_utils->FileSize(m_fname_fw_image) - m_footer_size;
    m_i_offset = m_fw_image_size - (1024 + m_footer_size);  // To check only the Last 512 byte Chunk of the IFWI for FUPH Header
...
unsigned int CloverviewPlusFW::GetFuphHeaderSize()
{
    ...
    else
    {
        ...
        fuphheadersize = FW_UPDATE_PROFILE_OLD_HDR_SIZE_CLVP;
    }
    return fuphheadersize;
}

unsigned int CloverviewPlusFW::GetDataChunckSize( unsigned int tempData)
{
...
void CloverviewPlusFW::LogError(int errorcode)
{
...
bool CloverviewPlusFW::InitFwImage(bool )
{
    int ret_code = 0;
    bool ret = true;
    this->m_utils->u_log(LOG_ENTRY, "%s", __FUNCTION__);
    ...
    try{
        ...
        FooterSizeInit();
        fw_image_size = fw_image_size - this->m_footer_size;
        ...
        if(FindFuphHeaderSignature()) {
            ...
            if ( IsSecondStageBIOS == 0 )
            {
                ...
                if(m_rom_patch_size > 0)
                {
                    ...
                    else //ROM Patch and VED FW
                    {
                        ...
                        if(endOfPadding > 0x1A6000)
                        {
                           //this is the padding after the rom patch but before the fup header
                            m_romPatchPostPaddingSize = endOfPadding - 0x1A6000;
                            endOfPadding = 0x1A6000;
                        }

                        //This is the padding before vedfw and after security firmware
                        m_romPatchPaddingSize = endOfPadding - totalIfwiSize;
                    }
                }
            ...
            if ( IsSecondStageBIOS == 0 )
            {
                ...
                if(m_romPatchPostPaddingSize > 0 )
                {
                    m_romPatchPostPadding = new UCHAR[m_romPatchPostPaddingSize];
                    if(!m_romPatchPostPadding)
					{
						if( fp_fw_image )
							fclose( fp_fw_image );
                        throw 5;
...
#if defined XFSTK_STATUS_SOCKET
            //status->sendmessage("Loading FW image buffers...");
#endif
            ...
            if ( IsSecondStageBIOS == 0 )
            {
                ...
                if(m_rom_patch_size > 0)
                {
                    ...
                    if(m_romPatchPostPaddingSize > 0)
                    {
                        //Read in the Padding Between ROM Patch and fup header
                        read_cnt = fread(m_romPatchPostPadding, sizeof(UCHAR), m_romPatchPostPaddingSize, fp_fw_image);
                        if(read_cnt != m_romPatchPostPaddingSize)
						{
							if( fp_fw_image )
								fclose( fp_fw_image );
                            throw 6;
...
bool CloverviewPlusFW::InitNoSize()
{
    bool ret = true;
    this->m_utils->u_log(LOG_ENTRY, "%s", __FUNCTION__);
...
dnx_data* CloverviewPlusFW::GetFwImageData(unsigned long index)
{
    if(index >= NUM_FW_DATA_CLVP)
        return NULL;
...
bool CloverviewPlusFW::CheckFile(char *filename)
{
    bool ret = true;
    this->m_utils->u_log(LOG_ENTRY, "%s", __FUNCTION__);
    FILE *fp;
...
void CloverviewPlusFW::FooterSizeInit()
{
    size_t location =0;
    size_t fupLoc = 0;

    this->m_footer_size = 0;
    fupLoc = this->m_utils->StringLocation(this->m_fname_fw_image,string("UPH$"),false);
    location = this->m_utils->StringLocation(this->m_fname_fw_image,string("$CFS"),false);

    if((location != std::string::npos) && (fupLoc != std::string::npos))
    {
        if(location > fupLoc)
        {
