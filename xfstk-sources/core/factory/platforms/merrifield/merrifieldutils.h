...
*/
#ifndef MERRIFIELDUTILS_H
#define MERRIFIELDUTILS_H
...
public:
    ...
    size_t StringLocation(const char* file,std::string str, bool front =true);
    size_t StringLocation(const char *buffer, std::string str, size_t sizeofBuffer);
    unsigned int dwordCheckSum(unsigned char *buff, int size);
...
private:
	MerrifieldUtils( const MerrifieldUtils& );
	MerrifieldUtils& operator=( const MerrifieldUtils& );
};

#endif // MERRIFIELDUTILS_H
