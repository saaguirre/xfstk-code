...
CPSTR Emmc_error_code_array [MAX_ERROR_CODE] = {
                    "Success",                                                                //0
                    "Connection error",                                                       //1
                    "Aborted by User",                                                        //2
...
ULONGLONG EmmcUtils::endian_swap(ULONGLONG x)
{
...
int EmmcUtils::strstr_lowercase_strip(char szBuff[], const char *keyword, int getChunkID,
                                                   bool RIMGSize, bool idrqData)
{
    ...
    if(found)
    {
        ...
        if(idrqData == true)
        {
            ...
            keyword_len += 32;
            //u_log(LOG_UTIL, "IDRQ Data is -->> ", szIdrqData);
        }
    }
    else
        result = -1;
...
ULONGLONG EmmcUtils::scan_string_for_protocol(char szBuff[])
{
    ULONGLONG keywords_as_ulonglong[MAX_ACK_CODE_EMMC] = {
        BULK_ACK_DxxM,
        BULK_ACK_DXBL,
        BULK_ACK_RDY$,
...
int EmmcUtils::index_of_keyword(char szBuff[], int start_index)
{
    int result = -1;
    int getChunkID = 0;
    const char * keywords[MAX_ACK_CODE_EMMC] = {
        "DxxM",
        "DXBL",
        "RDY$",
        "HLT$",
    ...
    for(int i=start_index; i< MAX_ACK_CODE_EMMC; i++){
        if((i == 10) || (i == 11) || (i == 12))
            getChunkID = 1;
        else
            getChunkID = 0;
...
void EmmcUtils::u_setstatuspfn(void(*physstatuspfn)(char* status, void* clientdata), void* clientdata)
{
    validstatuspfn = physstatuspfn;
    validstatusclientdata = clientdata;
}
void EmmcUtils::u_apistatus(char * message)
{
...
void EmmcUtils::u_log(uint32 logLevel, string message, ...)
{
    string fmtMsg;
...
void EmmcUtils::u_error(std::string message, ...)
{
...
void EmmcUtils::u_abort(std::string message, ...)
{
...
uint32 EmmcUtils::FileSize(char* filename)
{
    uint32 siz = 0;
...
std::string EmmcUtils::makestring(PUCHAR pbuf, uint32 size)
{
    string tmpStr;
    if(size < 25)
...
void EmmcUtils::SetUsbsn(char* usbsn)
{
