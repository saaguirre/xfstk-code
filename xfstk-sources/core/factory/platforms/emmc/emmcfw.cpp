...
EmmcFW::~EmmcFW()
{
    ...
    for(int i = 0; i < NUM_FW_DATA_EMMC; i++) {
       if(fw_data_set[i])
...
bool EmmcFW::Init(char* fname_dnx_fw, char* fname_fw_image, 
                      EmmcUtils* utils, unsigned long gpflags, bool )
{
    bool ret = true;
    m_utils = utils;
    ...
    if(InitNoSize()) {
        fw_data_set[FW_DATA_DNX0H_EMMC]->size = DNX_FW_SIZE_HDR_SIZE;
        fw_data_set[FW_DATA_DNX0H_EMMC]->data = m_dnx_os_size_hdr;
    }
    ...
    fw_data_set[FW_DATA_DNXH_EMMC]->size = m_dnx_fw_header_size;
    fw_data_set[FW_DATA_DNXH_EMMC]->data = m_dnx_fw_size_hdr;
    fw_data_set[FW_DATA_DNXD_EMMC]->size = m_dnx_fw_size;
    fw_data_set[FW_DATA_DNXD_EMMC]->data = m_dnx_fw;
    fw_data_set[FW_DATA_DNX0H_EMMC]->size = m_dnx_fw_header_size;
    fw_data_set[FW_DATA_DNX0H_EMMC]->data = m_dnx_os_size_hdr;
    return ret;
}

bool EmmcFW::InitDnxHdr()
{
    int ret_code = 1;
    bool ret = true;
...
void EmmcFW::InitFuphHeaderData()
{
...
bool EmmcFW::FindFuphHeaderSignature()
{
...
unsigned long EmmcFW::GetFuphHeaderSize()
{
    ...
    else
    {
        ...
        fuphheadersize = FW_UPDATE_PROFILE_OLD_HDR_SIZE_EMMC;
    }
    return fuphheadersize;
}

unsigned long EmmcFW::GetDataChunckSize( unsigned long tempData)
{
...
void EmmcFW::LogError(int errorcode)
{
    this->m_utils->u_log(LOG_ENTRY, "%s", __FUNCTION__);
    if(errorcode < MAX_ERROR_CODE)
        this->m_utils->u_abort("Error Code: %d - %s", errorcode, Emmc_error_code_array[errorcode]);
}

bool EmmcFW::InitFwImage(bool )
{
    int ret_code = 0;
    bool ret = true;
    this->m_utils->u_log(LOG_ENTRY, "%s", __FUNCTION__);
    ...
    try{
        ...
        if(FindFuphHeaderSignature()) {
            ...
            if(m_ifwi_size == 0x40000)//256KB (Emmc)
            {
            ...
            else // If Second stage BIOS is there
            {
                ...
                if(m_rom_patch_size > 0)
                {
					...
					m_rom_patch = new UCHAR[m_romPatchPaddingSize];
                    read_cnt = fread(m_rom_patch, sizeof(UCHAR), m_rom_patch_size, fp_fw_image);
                    if(read_cnt != m_rom_patch_size)
                    {
						if( fp_fw_image )
...
dnx_data* EmmcFW::GetFwImageData(unsigned long index)
{
    if(index >= NUM_FW_DATA_EMMC)
        return NULL;
...
bool EmmcFW::CheckFile(char *filename)
{
    bool ret = true;
    this->m_utils->u_log(LOG_ENTRY, "%s", __FUNCTION__);
    FILE *fp;
