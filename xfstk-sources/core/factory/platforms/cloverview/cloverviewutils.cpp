...
CPSTR Cloverview_error_code_array [MAX_ERROR_CODE] = {
                    "Success",                                                                //0
                    "Connection error",                                                       //1
                    "Aborted by User",                                                        //2
...
CloverviewUtils::CloverviewUtils()
{
...
ULONGLONG CloverviewUtils::endian_swap(ULONGLONG x)
{
...
int CloverviewUtils::strstr_lowercase_strip(char szBuff[], const char *keyword, int getChunkID,
...
ULONGLONG CloverviewUtils::scan_string_for_protocol(char szBuff[])
{
    ULONGLONG keywords_as_ulonglong[MAX_ACK_CODE_CLOVERVIEW] = {
        SERIAL_START,
...
int CloverviewUtils::index_of_keyword(char szBuff[], int start_index)
{
    int result = -1;
    int getChunkID = 0;
    const char * keywords[MAX_ACK_CODE_CLOVERVIEW] = {
        "SoTx",
    ...
    for(int i=start_index; i< MAX_ACK_CODE_CLOVERVIEW; i++){
        if((i == 10) || (i == 11) || (i == 12))
            getChunkID = 1;
        else
            getChunkID = 0;
...
void CloverviewUtils::u_setstatuspfn(void(*physstatuspfn)(char* status, void* clientdata), void* clientdata)
{
    validstatuspfn = physstatuspfn;
    validstatusclientdata = clientdata;
}
void CloverviewUtils::u_apistatus(char * message)
{
...
void CloverviewUtils::u_log(uint32 logLevel, string message, ...)
{
    string fmtMsg;
...
void CloverviewUtils::u_error(std::string message, ...)
{
...
void CloverviewUtils::u_abort(std::string message, ...)
{
...
uint32 CloverviewUtils::FileSize(char* filename)
{
    uint32 siz = 0;
...
std::string CloverviewUtils::makestring(PUCHAR pbuf, uint32 size)
{
    string tmpStr;
    if(size < 25)
...
void CloverviewUtils::SetUsbsn(char* usbsn)
{
