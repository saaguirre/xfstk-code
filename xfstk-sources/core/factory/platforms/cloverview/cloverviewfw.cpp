...
//CloverviewFW Implementation
CloverviewFW::CloverviewFW()
{
    m_fname_dnx_fw = NULL;
...
CloverviewFW::~CloverviewFW()
{
    ...
    for(int i = 0; i < NUM_FW_DATA_CLV; i++) {
       if(fw_data_set[i])
       {
...
bool CloverviewFW::Init(char* fname_dnx_fw, char* fname_fw_image, 
                      CloverviewUtils* utils, unsigned long gpflags, bool enable_ifwi_wipe)
{
    bool ret = true;
    m_utils = utils;
    ...
    if(InitNoSize()) {
        fw_data_set[FW_DATA_DNX0H_CLV]->size = DNX_FW_SIZE_HDR_SIZE;
        fw_data_set[FW_DATA_DNX0H_CLV]->data = m_dnx_os_size_hdr;
    }
    ...
    fw_data_set[FW_DATA_DNXH_CLOVERVIEW]->size = m_dnx_fw_header_size;
    fw_data_set[FW_DATA_DNXH_CLOVERVIEW]->data = m_dnx_fw_size_hdr;
    fw_data_set[FW_DATA_DNXD_CLV]->size = m_dnx_fw_size;
    fw_data_set[FW_DATA_DNXD_CLV]->data = m_dnx_fw;
    fw_data_set[FW_DATA_DNX0H_CLV]->size = m_dnx_fw_header_size;
    fw_data_set[FW_DATA_DNX0H_CLV]->data = m_dnx_os_size_hdr;
    return ret;
}

bool CloverviewFW::InitDnxHdr()
{
    int ret_code = 1;
    bool ret = true;
...
void CloverviewFW::InitFuphHeaderData()
{
...
bool CloverviewFW::FindFuphHeaderSignature()
{
...
unsigned int CloverviewFW::GetFuphHeaderSize()
{
    ...
    else
    {
        ...
        fuphheadersize = FW_UPDATE_PROFILE_OLD_HDR_SIZE_CLV;
    }
    return fuphheadersize;
}

unsigned int CloverviewFW::GetDataChunckSize( unsigned int tempData)
{
...
void CloverviewFW::LogError(int errorcode)
{
...
bool CloverviewFW::InitFwImage(bool )
{
    int ret_code = 0;
    bool ret = true;
    this->m_utils->u_log(LOG_ENTRY, "%s", __FUNCTION__);
...
dnx_data* CloverviewFW::GetFwImageData(unsigned long index)
{
    if(index >= NUM_FW_DATA_CLV)
        return NULL;
...
bool CloverviewFW::CheckFile(char *filename)
{
    bool ret = true;
    this->m_utils->u_log(LOG_ENTRY, "%s", __FUNCTION__);
    FILE *fp;
